...
export default class WaveGenerator {
	...
	public generate(svgCanvasClassName = '.canvas'): any {
		...
		const wave = (start: XYPoint, end: XYPoint, gradient: any) => {
			...
			const pathData = `${spline(points, 1, false)}L ${
				end.x
			} ${height} L ${start.x} ${height} Z`;
			svg.path(pathData).addClass('path').attr('fill', gradient);
		};

		const { numWaves } = config;
		const base = tinycolor(`hsl(${random(0, 360)}, 65%, 55%)`);
		this.colors = base.analogous(6);

		svg.rect(width, height).fill(
			random(this.colors).clone().darken(40).toString()
		);
		...
		for (let i = 0; i < numWaves; i++) {
			...
			if (i > 3) {
				color = color
					.darken(random(2 * darkenFactor, 4 * darkenFactor))
					.desaturate(random(10, 20));
			}
